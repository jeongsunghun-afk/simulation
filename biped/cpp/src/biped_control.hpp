...
struct BipedControl {
  ...
  // ★★2026-09-22 **실기 측정자세로 교체**(사용자 지정). Pi(192.168.10.19) /dev/shm/biped_state.json
  //   q_leg_deg 12샘플 평균(mode=jog, 정지). flat_home.cpp 의 대칭 해가 아니라 **로봇이 실제로
  //   선 자세**를 flat home 으로 못박는다 → home·WBIC posture·posture-hold 기준이 실기와 일치.
  //   ⚠좌우 비대칭이 그대로 들어감: 왼발 -43.9°(ch77.9) vs 오른발 -55.0°(ch91.0), Δ13.1°.
  //     = 왼 foot 영점이동/toe-up(벨트 이력). flat_home.cpp 대칭가정(밑창수평·CoM=밑창중심)과
  //     어긋나므로 이 자세의 CoM/지지면은 재검증 필요. 왼발 벨트 수리·평평화 후 다시 캡처할 것.
  //   구값(대칭·flat_home.cpp): {0,0.064256,-0.416657,-1.043858, 0,0.064256,-0.416657,-1.043858}
  double Qflat8[8]={-0.019097,0.054542,-0.365981,-0.766738, 0.017671,0.054876,-0.363741,-0.959742};
  // ── 파라미터 (Python 동일) ──
