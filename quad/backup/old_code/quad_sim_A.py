...
def mode_trot():
    ...
    def ctrl():
        ...
        for i in sw:                                        # swing 발끝 작업공간 목표(p,v)
            ...
            # ★A2: swing 속도 피드포워드 클립 상향. 고속(V=1.2)서 발은 ~3.6m/s 필요한데 ±1.0은 추종지연→포화.
            #   궤적(Zeng spline)은 매끄러워 차분속도가 정확 → 클립은 노이즈가드로만(높게).
            _vc = float(os.environ.get('SWING_VCLIP', '1.0'))   # 상향=악화 확인(클립=안정화 역할). 기본 baseline유지
            v_tgt = np.clip((p_tgt - pv) / dt, -_vc, _vc) if pv is not None else np.zeros(3)
